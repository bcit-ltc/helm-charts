...
1. List the available registry tags
    ...
    https://api.github.com/orgs/bcit-ltc/packages/container/dataset-retriever/versions \
...
1. Pull and inspect the helm chart:
    ...
    ```console
    helm pull --untar oci://ghcr.io/bcit-ltc/dataset-retriever --version {VERSION}
    ```

1. Install the chart:

    ```console
    helm install dataset-retriever .
    ```
...
| frontend.image.repository | string | `"bcit-ltc/dataset-retriever"` | Frontend image repository |
...
| frontend.name | string | `"dataset-retriever"` | The name of the frontend container to create. If empty uses "frontend" |
...
| global.name | string | `"dataset-retriever"` | Authoritative name |
