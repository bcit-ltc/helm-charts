...
legacy-soh is a legacy multimedia application.

**Homepage:** <https://legacy-soh.ltc.bcit.ca>
...
1. List the available registry tags
    ...
    https://api.github.com/orgs/bcit-ltc/packages/container/legacy-soh/versions \
...
1. Pull and inspect the helm chart:
    ...
    ```console
    helm pull --untar oci://ghcr.io/bcit-ltc/legacy-soh --version {VERSION}
    ```

1. Install the chart:

    ```console
    helm install legacy-soh .
    ```
...
| assets.ref | string | `"ghcr.io/bcit-ltc/legacy-soh-assets"` | Reference to the asset OCI image |
...
| frontend.image.repository | string | `"bcit-ltc/legacy-soh"` | Frontend image repository |
...
| frontend.name | string | `"legacy-soh"` | The name of the frontend container to create. If empty uses "frontend" |
...
| global.name | string | `"legacy-soh"` | Authoritative name |
...
| initContainer.image.repository | string | `"bcit-ltc/legacy-soh"` | initContainer image repository |
