...
legacy-sob is a legacy multimedia application.

**Homepage:** <https://legacy-sob.ltc.bcit.ca>
...
1. List the available registry tags
    ...
    https://api.github.com/orgs/bcit-ltc/packages/container/legacy-sob/versions \
...
1. Pull and inspect the helm chart:
    ...
    ```console
    helm pull --untar oci://ghcr.io/bcit-ltc/legacy-sob --version {VERSION}
    ```

1. Install the chart:

    ```console
    helm install legacy-sob .
    ```
...
| assets.ref | string | `"ghcr.io/bcit-ltc/legacy-sob-assets"` | Reference to the asset OCI image |
...
| frontend.image.repository | string | `"bcit-ltc/legacy-sob"` | Frontend image repository |
...
| frontend.name | string | `"legacy-sob"` | The name of the frontend container to create. If empty uses "frontend" |
...
| global.name | string | `"legacy-sob"` | Authoritative name |
...
| initContainer.image.repository | string | `"bcit-ltc/legacy-sob"` | initContainer image repository |
