...
course-production-site is a public landing page to course production services and resources.

## Quickstart

<!-- ```bash
helm install course-production-site ./nginx-unprivileged-app   --set image.tag=1.27-alpine   --set ingress.enabled=true   --set defaultDomain=ltc.bcit.ca
``` -->

<!-- Then visit: `http://course-production-site.ltc.bcit.ca` (or use port-forward from the NOTES). -->
...
| ingress.host | string | `"course-production-site.ltc.bcit.ca"` |  |
