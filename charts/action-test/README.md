...
action-test is a test repo for github action

**Homepage:** <https://action-test.ltc.bcit.ca>
...
1. List the available registry tags
    ...
    https://api.github.com/orgs/bcit-ltc/packages/container/action-test/versions \
...
1. Pull and inspect the helm chart:
    ...
    ```console
    helm pull --untar oci://ghcr.io/bcit-ltc/action-test --version {VERSION}
    ```

1. Install the chart:

    ```console
    helm install action-test .
    ```
...
| frontend.image.repository | string | `"bcit-ltc/action-test"` | Frontend image repository |
...
| frontend.name | string | `"action-test"` | The name of the frontend container to create. If empty uses "frontend" |
...
| global.name | string | `"action-test"` | Authoritative name |
