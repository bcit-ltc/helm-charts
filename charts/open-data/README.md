...
**Homepage:** <https://open-data.ltc.bcit.ca>
...
1. List the available registry tags
    ...
    https://api.github.com/orgs/bcit-ltc/packages/container/open-data/versions \
...
1. Pull and inspect the helm chart:
    ...
    ```console
    helm pull --untar oci://ghcr.io/bcit-ltc/open-data --version {VERSION}
    ```

1. Install the chart:

    ```console
    helm install open-data .
    ```
...
| frontend.image.repository | string | `"bcit-ltc/open-data"` | Frontend image repository |
...
| frontend.name | string | `"open-data"` | The name of the frontend container to create. If empty uses "frontend" |
...
| global.name | string | `"open-data"` | Authoritative name |
