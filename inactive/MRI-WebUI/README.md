...
**Homepage:** <https://MRI-WebUI.ltc.bcit.ca>
...
1. List the available registry tags
    ...
    https://api.github.com/orgs/bcit-ltc/packages/container/MRI-WebUI/versions \
...
1. Pull and inspect the helm chart:
    ...
    ```console
    helm pull --untar oci://ghcr.io/bcit-ltc/MRI-WebUI --version {VERSION}
    ```

1. Install the chart:

    ```console
    helm install MRI-WebUI .
    ```
...
| frontend.image.repository | string | `"bcit-ltc/MRI-WebUI"` | Frontend image repository |
...
| frontend.name | string | `"MRI-WebUI"` | The name of the frontend container to create. If empty uses "frontend" |
...
| global.name | string | `"MRI-WebUI"` | Authoritative name |
